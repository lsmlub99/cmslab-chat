...
Vercel 은 요청이 없으면 함수 인스턴스를 정리합니다. 다음 접속자는 함수 기동과
Supabase 연결 비용을 전부 부담하므로 첫 응답이 느려집니다.
`/api/ping` 을 주기적으로 호출해 두면 이 지연이 사라집니다.

```
https://<배포주소>/api/ping
```

`select 1` 만 실행하는 가벼운 엔드포인트입니다.

### 설정 방법 (약 1분)

[cron-job.org](https://cron-job.org) 무료 계정으로 충분합니다.

1. 가입 후 **Create cronjob**
2. **URL**: `https://<배포주소>/api/ping`
3. **Schedule**: `Every 5 minutes`
4. Save

UptimeRobot(5분 간격 무료)도 같은 용도로 쓸 수 있습니다.

### 왜 Vercel Cron 이나 GitHub Actions 를 안 쓰는가

- **Vercel Cron**: Hobby 플랜은 **하루 1회**만 실행됩니다. 워밍 용도로는 못 씁니다.
- **GitHub Actions**: 비공개 저장소는 월 2,000분 무료인데, 실행 시간이 몇 초라도
  **1분 단위로 청구**됩니다. 15분 간격이면 월 2,880분이라 무료 한도를 넘습니다.
  예약 실행이 최대 30분까지 밀리기도 하고, 저장소가 60일 조용하면 자동 비활성화됩니다.

### 안 해도 되나

해도 되고 안 해도 됩니다. 화면(HTML·JS)은 CDN 에서 나가므로 콜드 스타트와 무관하게
항상 즉시 뜹니다. 느려지는 것은 API 응답뿐이고, 지금은 실패하지 않고 몇 초 늦어질
뿐입니다. 사용자가 많아 항상 트래픽이 있으면 자연히 따뜻하게 유지됩니다.
